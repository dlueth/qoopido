const { toArray } = require('../dist/umd/index');
...
describe('toArray()', () => {
    ...
    test('should successfully convert array-like-objects to array', () => {
        ...
        expect((function() { return toArray(arguments); }).apply(null,args )).toEqual(args);
    });

    test('should slice from "start" when passed', () => {
        const args = [ 'argument1', 'argument2', 'argument3' ];

        expect((function() { return toArray(arguments, 1); }).apply(null,args )).toEqual(args.slice(1));
    });
});
